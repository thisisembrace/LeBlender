    ...
    public class Helper
    {
        ...
        internal static PublishedContentType GetTargetContentType()
        {
            ...
            else
            {
                int contenId = 0;
                if (HttpContext.Current.Request["id"] != null)
                    contenId = int.Parse(HttpContext.Current.Request["id"]);
                else
                {
                    if (HttpContext.Current.Session["NodeId"] != null)
                        contenId = int.Parse(HttpContext.Current.Session["NodeId"].ToString());
